  ...
  class API
    ...
    class Order
      def self.order(payload)
        res = Helper.request('/v3/orders', { data: payload }, 'POST')
        if res.code == 402
      ...
      def self.order_detail(id)
        Helper.request("/v3/orders/#{id}", {}, 'GET')
      end

      def self.driver_details(id, driver_id)
        Helper.request("/v3/orders/#{id}/drivers/#{driver_id}", {}, 'GET')
      end

      def self.driver_location(id, driver_id)
        Helper.request("/v3/orders/#{id}/drivers/#{driver_id}/location", {}, 'GET')
      end

      def self.cancel_order(id)
        Helper.request("/v3/orders/#{id}", {}, 'DELETE')
      end
    end
  end
end
